...
int main()
{
    ...
    ll start = 0;
    ll current_block_result = 0;

    for (ll pos = 0; pos < input.at(0).size(); pos++)
    {
        bool any_digits = false;
        bool all_spaces = true;
        auto sign = (*input.rbegin()).at(start);
        assert(sign == '+' || sign == '*');
        ...
        if (pos == start)
        {
            if (sign == '*')
            {
                current_block_result = 1;
            }
            else
            {
                current_block_result = 0;
            }
        }

        for (ll line = 0; line < input.size() - 1; line++)
        {
            any_digits |= isdigit(input.at(line).at(pos));
            all_spaces &= (input.at(line).at(pos) == ' ');
        }
        ...
        if (any_digits)
        {
            ...
            for (ll line = 0; line < input.size() - 1; line++)
            {
                if (isdigit(input.at(line).at(pos)))
                {
                    column_digit = column_digit * 10 + (input.at(line).at(pos) - '0');
                }
            }

            if (sign == '+')
            {
                current_block_result += column_digit;
            }
            else
            {
                current_block_result *= column_digit;
        ...
        if (all_spaces || pos == input.at(0).size() - 1)
        {
            result0 += current_block_result;
            start = pos + 1;
        }
    }
